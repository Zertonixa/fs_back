...
async def login(
    payload: TelegramAuthIn, response: Response, auth: AuthService = Depends(get_auth_service)
) -> TokenOut:
...
async def me(user: Users = Depends(get_current_user)) -> UserOut:
    return UserOut(id=str(user.id), telegram_id=user.telegram_id, username=user.username)
