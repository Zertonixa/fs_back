...
router = APIRouter(prefix="/users", dependencies=[Depends(get_current_user_id)])


@router.get("/", status_code=http_status.HTTP_200_OK)
async def get_all_users(user_service: UserService = Depends(get_users_service)) -> list[User]:
